...
from customized_addons.req_res_handling.response import (
    valid_response, invalid_response)
from customized_addons.vkclub.token import validate_token
...
class VkclubController(http.Controller):
    ...
    @validate_token
    @http.route('/orders/topup', type="json", auth="none", methods=["POST"], csrf=False)
    def create_top_up_journal(self):
        payload = json.loads(request.httprequest.data)
        amount = payload.get('amount', False)
        ...
        remark = payload.get('remark', False)
        cash_out = payload.get('cash_out', False)
        print(amount, partner_id, payload)
        if amount and partner_id:
            ...
            print("===================== successfully nearly creating move val object =========================",
                  partner_id)
