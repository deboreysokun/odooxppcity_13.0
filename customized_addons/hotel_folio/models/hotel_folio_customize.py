...
class HotelReceiptNo(models.Model):
    ...
    customer_type = fields.Selection(
        [
            ('fit', "FIT"),
            ('group',"Group"),
            ('partner', "Corporate's Partner"),
            ('sales',"Corporate's Sale"),
        ],
        related="reservation_id.customer_type",
        readonly=True
    )
...
class HotelReceiptNoInInvoice(models.Model):
    ...
    fol_id = fields.Many2one('hotel.folio',string="Folio", on_delete="restrict", )
    ...
    customer_type = fields.Selection(
        ...
        related="fol_id.customer_type",
        readonly=True
    )
    @api.model
    def update_existing_records(self):
        # Find records where fol_id is empty and fol_no_inv is not empty
        records_to_update = self.search([('fol_id', '=', False), ('fol_no_inv', "!=", False)])

        print("Number of Records", len(records_to_update))

        for record in records_to_update:

            record.fol_id = self.env['hotel.folio'].search([('name', '=', record.fol_no_inv)], limit=1)
